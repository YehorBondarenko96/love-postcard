...
const TimerForm = () => {
  ...
  useEffect(() => {
    if (screenWidth > 0 && firstMassageRef.current && allDivRef.current && secondAllDivRef.current
      && generalTextRef.current && yearsRef.current && monthsRef.current
      && daysRef.current && hoursRef.current && minutesRef.current
      && secondsRef.current && questionTextRef.current && answerFirstMessageRef.current
    ...
    <div ref={allDivRef} className={css.allDiv}>
      ...
      <button ref={heartRef} className={`${css.button} ${css.heart}`}>
        <div className={css.heartImageDiv}>
           <Heart size={screenWidth > 0 ? screenWidth/5 : 200} color="red" />
        ...
        <button ref={brokenHeartRef} className={`${css.button} ${css.brokenHeart}`}>
          <div className={css.heartImageDiv}>
            <BrokenHeart size={screenWidth > 0 ? screenWidth / 5 : 200} />
